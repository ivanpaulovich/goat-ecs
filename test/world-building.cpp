...
TEST(WorldTest, ComponentsBuilding) {
  ...
  Position pos1 =
      components.getComponents<Position>(POSITION_KEY)[POSITION_ELEMENT_ID];
...
TEST(WorldTest, WorldBuilding) {
  ...
  w.newEntity()->include<Velocity>({0, 3})->include<Position>({2, 1})->apply();

  auto healthObjects = w.newQuery()->include<Health>()->apply()->getIndex();

  for (auto itr : *healthObjects) {
    auto health = w.loadEntity(itr)->get<Health>();
